...
export function useOfficeData(profile) {
  ...
  const loadAll = useCallback(async () => {
    setLoading(true);
    try {
      const [c, i, t, dstatus, s, exp, log, prof] = await Promise.allSettled([
      ...
      const cRes = val(c), iRes = val(i), tRes = val(t), dRes = val(dstatus), sRes = val(s), expRes = val(exp), logRes = val(log), profRes = val(prof);
  ...
  return {
    clients, invoices, expenses, tasks, profiles, activityLog, declarationStatus, settings, loading, error, setError,
