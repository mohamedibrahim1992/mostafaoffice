...
export default function InvoicesPage() {
  ...
  const [form, setForm] = useState({ client_id: "", amount: "", status: "معلقة", description: "", date: todayISO() });
  ...
  const save = async () => {
    ...
    setForm({ client_id: "", amount: "", status: "معلقة", description: "", date: todayISO() });
    setModal(false);
  };
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <Btn onClick={() => setModal(true)}><Plus size={16} /> إضافة فاتورة</Btn>
      </div>
          ...
          <tbody>
            {rows.map((i) => (
              <tr key={i.id} className="border-b border-slate-100 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/30">
                ...
                <td className="px-3 py-2"><button onClick={() => remove(i)} className="p-1.5 hover:bg-rose-50 dark:hover:bg-rose-900/30 text-rose-500 rounded-lg"><Trash2 size={14} /></button></td>
              </tr>
            ))}
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="إضافة فاتورة جديدة">
